// International dialling codes for the phone field on the Customize Your Tour
// form (/customize, linked as "Plan My Trip").
//
// A number arriving without a country code is often unusable: "01234567890"
// could be Egypt, the UK or half a dozen other countries, and the team has to
// guess or email back to ask. Requiring the code up front makes every number
// callable on receipt.
//
// The full list rather than a shortlist of popular markets — enquiries come
// from anywhere, and a traveler whose country is missing from the dropdown
// cannot submit the form at all.

export type CountryDialCode = {
  /** ISO 3166-1 alpha-2, used to derive the flag and to key the option. */
  iso: string;
  name: string;
...
/**
 * The flag emoji for an ISO alpha-2 code, derived rather than stored: each
 * letter maps to its regional indicator symbol, so "EG" becomes 🇪🇬.
 */
export function flagFor(iso: string): string {
  return String.fromCodePoint(...[...iso.toUpperCase()].map((c) => 0x1f1e6 + c.charCodeAt(0) - 65));
}

// Sorted by country name so the dropdown reads alphabetically. Several
// countries share a dialling code (+1 across North America and the Caribbean,
// +7 for Russia and Kazakhstan); each keeps its own row so a traveler can find
// their own country by name.
export const countryDialCodes: CountryDialCode[] = [
  ...
  { iso: "PS", name: "Palestinian Territories", dial: "+970" },
